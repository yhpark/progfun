...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors filter { case (b, history) => !(explored contains(b)) }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      initial.head #:: from(initial.tail #::: newNeighborsOnly(neighborsWithHistory(initial.head._1, initial.head._2), newExplored), newExplored)
    }
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter { case (b, history) => done(b) }
