...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def this(elem: Tweet) = this(elem, new Empty, new Empty)

  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    right.filterAcc(p, left.filterAcc(p, if (p(elem)) acc incl elem else acc))

  lazy val mostRetweeted: Tweet = {
    val lm = if (!left.isEmpty) left.mostRetweeted.retweets else -1
    val rm = if (!right.isEmpty) right.mostRetweeted.retweets else -1

    if (lm >= elem.retweets && lm >= rm) left.mostRetweeted
    else if (elem.retweets >= lm && elem.retweets >= rm) elem
...
object GoogleVsApple {
  ...
  def containsAny(keywords: List[String], t: Tweet) =
    keywords.foldRight(false)((keyword: String, b: Boolean) => if (b) true else t.text.contains(keyword))

  lazy val googleTweets: TweetSet =
    TweetReader.allTweets.filter(t => containsAny(google, t))

  lazy val appleTweets: TweetSet =
    TweetReader.allTweets.filter(t => containsAny(apple, t))
